...
package com.dayani.m.thirdparty.google.gnsslogger;
...
public class MeasurementProvider {
  ...
  public static final int ANDROID_GNSS_API_VERSION = Build.VERSION_CODES.N;
...
//              }
  ...
  private final GnssMeasurementsEvent.Callback gnssMeasurementsEventListener =
          getGnssMeasurementsEventListener();

  private GnssMeasurementsEvent.Callback getGnssMeasurementsEventListener() {

    if (SDK_INT >= ANDROID_GNSS_API_VERSION) {
  ...
  private final GnssNavigationMessage.Callback gnssNavigationMessageListener =
          getGnssNavigationMessageListener();

  private GnssNavigationMessage.Callback getGnssNavigationMessageListener() {
  ...
  private final GnssStatus.Callback gnssStatusListener = getGnssStatusListener();

  private GnssStatus.Callback getGnssStatusListener() {
  ...
  private final OnNmeaMessageListener nmeaListener = getNmeaListener();

  private OnNmeaMessageListener getNmeaListener() {

    if (SDK_INT >= ANDROID_GNSS_API_VERSION) {
      return new OnNmeaMessageListener() {
        @Override
